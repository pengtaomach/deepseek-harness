...
The cards this package ships cover the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and one aggregated web-search card (`web-search-deepseek`) that selects the search provider and edits that provider's controls.
