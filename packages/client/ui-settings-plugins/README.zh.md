...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`），以及一张聚合的网页搜索卡片（`web-search-deepseek`），它选择搜索提供方并编辑该提供方的控件。
...
DeepSeek 搜索卡片编辑提供方的接口地址、搜索请求所用的模型以及单次请求的搜索次数预算；把文本字段清空并保存即清除覆盖，使该字段重新继承部署的组装默认值。
