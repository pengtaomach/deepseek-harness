/**
 * The PPIO search card's staged form over the `web-search-ppio` settings
 * namespace.
 *
...
/**
 * Namespace of the PPIO search provider. Spelled here rather than imported: a
 * client package must not depend on a Host package.
 */
export const WEB_SEARCH_PPIO_NS = 'web-search-ppio'

/** Credential reference the provider resolves when the section names none. */
const DEFAULT_API_KEY_REF = 'PPIO_API_KEY'

/** Form field the credential control stages under. */
const API_KEY_FIELD = 'apiKey'

/** The search-provider fields this card edits. */
export interface WebSearchPpioSettings {
...
/** What the PPIO search card renders. */
export interface WebSearchPpioCardState extends CardShell {
  /** Provider endpoint. */
...
/** The registration-side face the PPIO search card's slot entry injects. */
export interface WebSearchPpioCardFace extends CardActions {
  hooks: {
    /** Card snapshot bound by the renderer as useWebSearchPpioCard. */
    webSearchPpioCard: SnapshotStore<WebSearchPpioCardState>
  }
}

/** Bridges the `web-search-ppio` scope and the credentials domain onto the card. */
export class WebSearchPpioCardController {
  private readonly form: CardForm<WebSearchPpioSettings>
  private readonly store: SnapshotStore<WebSearchPpioCardState>
  private credential: CredentialState = { ref: '', configured: false, writable: true }

  /**
   * @param scope - the bound settings scope for the `web-search-ppio` namespace.
   * @param api - wire face used for the credential the section references.
   */
  constructor(
    private readonly scope: SettingsScope<WebSearchPpioSettings>,
  ...
  private projection(): WebSearchPpioCardState {
    return {
      ...this.form.shell(),
   ...
   */
  inject(): WebSearchPpioCardFace {
    return { hooks: { webSearchPpioCard: this.store }, ...this.form.actions() }
  }
 ...
 */
function refOf(snapshot: SettingsScopeSnapshot<WebSearchPpioSettings>): string {
